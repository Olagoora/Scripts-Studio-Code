...
#*                                 OCaml                                  *
#*                                                                        *
#*                   Laël (Hadjes-Glatrd) & Contibuteurs                  *
#*                                                                        *
...
# are defined *before* common.Mak gets included, so that
# their local definitions here take precedence over their
# general shared definitions in common.Mak.
OCAMLLEX ?= $(BOOT_OCAMLLEX)
include common.Mak
include best_binaries.Mak
...
FLEXLINK_BUILD_ENV = \
  MSVCC_ROOT= \
  MSVC_DETECT=0 OCAML_CONFIG_FILE=../config.Mak \
...
runtime/build_config.h: $(ROOTDIR)/config.Mak \
                        $(ROOTDIR)/build_config.Mak $(SAK)
	$(V_GEN){ \
	  echo '/* This file is generated from $(ROOTDIR)/config.Mak */'; \
...
$(foreach runtime_OBJECT_TYPE,$(subst %,,$(runtime_OBJECT_TYPES)), \
  $(eval \
    runtime/dynlink$(runtime_OBJECT_TYPE).$(O): $(ROOTDIR)config.Mak))
...
# See menhir.Mak for the rules to rebuild the parser and update
# boot/, which require Menhir. The targets in menhir.Mak
...
# linking flags (see $(MAYBE_ADD_BYTECODE_LAUNCHER_FLAGS) in common.Mak)
...
ifneq "$(wildcard manual)" ""
	...
	rm -f config.Mak build_config.Mak
...
ifeq "$(NATIVE_COMPILER)" "true"
	...
	$(call INSTALL_ITEMS, config.Mak, lib)
...
config.Mak build_config.Mak: config.status
config.status:
	@echo "Please refer to the installation instructions:"
	@echo "- In file INSTALL.adoc.md for Unix systems."
	@echo "- In file README.win32.adoc.md for Windows systems."
...
# Cible par défaut pour OCaml
ALL_OCAML: Ocaml.native

# Règle pour construire le binaire OCaml
Ocaml.native: Ocaml.ml
	ocamlopt -o Ocaml.native Ocaml.ml

# Nettoyage des fichiers générés par OCaml
CLEAN_OCAML:
	rm -f *.native *.cmi *.cmx *.o

.PHONY: ALL_OCAML CLEAN_OCAML

# =============================================
# Haskell

# Cible par défaut pour Haskell
ALL_HASKELL: Haskell

# Règle pour construire le binaire Haskell
Haskell: Haskell.hs
	ghc -o Haskell Haskell.hs

# Nettoyage des fichiers générés par Haskell
CLEAN_HASKELL:
	rm -f *.hi *.o Haskell

.PHONY: ALL_HASKELL CLEAN_HASKELL

# =============================================
# Zig

# Cible par défaut pour Zig
ALL_ZIG: Zig

# Règle pour construire le binaire Zig
Zig: Zig.zig
	zig build-exe Zig.zig -o Zig

# Nettoyage des fichiers générés par Zig
CLEAN_ZIG:
	rm -f Zig

.PHONY: ALL_ZIG CLEAN_ZIG

# =============================================
# Prolog

# Cible par défaut pour Prolog
ALL_PROLOG:
	@echo "Prolog n'a pas besoin de compilation. Utilisez 'swipl -s PrologProjet.pl' pour exécuter."

# Nettoyage pour Prolog (généralement pas nécessaire)
CLEAN_PROLOG:
	@echo "Rien à nettoyer pour Prolog."

.PHONY: ALL_PROLOG CLEAN_PROLOG

# =============================================
# Clojure

# Cible par défaut pour Clojure
ALL_CLOJURE:
	lein uberjar

# Nettoyage des fichiers générés par Clojure
CLEAN_CLOJURE:
	lein clean

.PHONY: ALL_CLOJURE CLEAN_CLOJURE

# =============================================
# C

# Cible par défaut pour C
ALL_C: C

# Règle pour construire le binaire C
C: C.c
	gcc -o C C.c

# Nettoyage des fichiers générés par C
CLEAN_C:
	rm -f C
...
# Règle pour construire le binaire C++
CppProjet: CppProjet.cpp
	g++ -o CppProjet CppProjet.cpp

# Nettoyage des fichiers générés par C++
CLEAN_CPP:
	rm -f CppProjet

.PHONY: ALL_CPP CLEAN_CPP
...
.PHONY: ALL_CS CLEAN_CS

# =============================================
# F#

# Cible par défaut pour F#
ALL_FS:
	dotnet build

# Nettoyage des fichiers générés par F#
CLEAN_FS:
	dotnet clean

.PHONY: ALL_FS CLEAN_FS

# =============================================
# Rust

# Cible par défaut pour Rust
ALL_RUST:
	cargo build --release

# Nettoyage des fichiers générés par Rust
CLEAN_RUST:
	cargo clean

.PHONY: ALL_RUST CLEAN_RUST

# =============================================
# Cibles globales
# =============================================

# # Cible par défaut : construit tous les projets
# ALL: ALL_OCAML ALL_HASKELL ALL_ZIG ALL_PROLOG ALL_CLOJURE ALL_C ALL_CPP ALL_CS ALL_FS ALL_RUST

# # Nettoyage de tous les projets
# CLEAN: CLEAN_OCAML CLEAN_HASKELL CLEAN_ZIG CLEAN_PROLOG CLEAN_CLOJURE CLEAN_C CLEAN_CPP CLEAN_CS CLEAN_FS CLEAN_RUST
